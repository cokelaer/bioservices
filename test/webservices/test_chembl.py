...
def test_filters(chembl):
    ...
    res1 = chembl.get_molecule(limit=20, filters="molecule_properties__mw_freebase__lte=300")
    assert len(res1) == 20

    # test with one filter in a list
    res2 = chembl.get_molecule(limit=20, filters=["molecule_properties__mw_freebase__lte=300"])
    assert len(res2) == 20
    ...
    res = chembl.get_molecule(
        limit=20, filters=["molecule_properties__mw_freebase__lte=300", "pref_name__iendswith=nib"]
    )
    assert len(res)
    for this in res:
...
def test_ordering(chembl):
    ...
    # double key
    data1 = [
        x["molecule_properties"]["alogp"] for x in chembl.order_by(res, "molecule_properties__alogp", ascending=True)
    ]
    assert data1[0] < data1[1]

    data2 = [
        x["molecule_properties"]["alogp"] for x in chembl.order_by(res, "molecule_properties__alogp", ascending=False)
    ]
