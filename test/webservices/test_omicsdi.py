...
def test_dataset_domain_accession_files(omicsdi):
    response = omicsdi.dataset_domain_accession_files(accession="PXD000210", domain="pride", position=1)
...
def test_statistics_omics_by_year(omicsdi):
    ...
    assert len(response)
    assert keys_exists(response[0], ("year", "genomics", "metabolomics", "proteomics", "transcriptomics"))
